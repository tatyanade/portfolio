...
function getValues(){
	...
	pattern.attributes.backNeckType = random(['curve', 'straight', 'square'])
	...
	pattern.attributes.hasNeckDartBack = random(0,100) < 40

	pattern.attributes.buttonLocation = random(['left', 'right', 'both'])
...
function makeWaistDarts(){
	...
	if(pattern.body.waist.y + height/2 > pattern.len){
		pattern.attributes.hasBackWaistDart = false;
		pattern.attributes.hasFrontWaistDart = false;
	}
}
...
function makeBottomDarts(){
 	...
 	x1= backx - width/2
 	y1= pattern.back.bottomIn.y//, pattern.back.bottomOut.y])
 	x2= backx
 	y2= y1 - height/2
 	x3= backx + width/2
 	y3=y1
 	...
 	x1= frontx - width/2
 	y1= pattern.front.bottomIn.y//, pattern.front.bottomOut.y])
 	x2= frontx
 	y2= y1 - height/2
 	x3= frontx + width/2
 	y3= y1
...
function drawBackBottomDart(){
	fill(0)
	if(pattern.attributes.hasBottomDartsBack){
	...
	if(pattern.attributes.hasBackWaistDart){
		if(pattern.body.waist.y + pattern.body.waistDart.dy > pattern.len){
...
function drawBackNeckDart(){
		if(pattern.attributes.hasNeckDartBack){
...
function drawFrontBottomDart(){
		if(pattern.attributes.hasBottomDartsFront){
...
function drawLapels(){
	if(pattern.attributes.hasLapels == true){
		...
		yoffset = padding*2 + pattern.front.bottomIn.y;
...
function drawButtons(side){
	if ( (pattern.attributes.buttonLocation == 'both') || 
		 (pattern.attributes.buttonLocation == side)){
		fill(0)
		if (pattern.attributes.hasButtons == true){
...
function redoCoattails(){
	pattern.attributes.hasCoattails = (random(0,100) < 70 ) && (pattern.attributes.lenType != 'long')
	makeCoattails()
...
function redoDarts(){
	...
	pattern.attributes.hasBottomDartsBack = random(0,100) < 30
	pattern.attributes.hasBottomDartsFront =  random(0,100) < 30
	pattern.attributes.hasNeckDartBack =  random(0,100) < 30
	makeDarts();
}